...
window.NFLSYNC=(function(){
  const CONF='sync.json', PROP_KEY='props_2026_v1', LIVE_KEY='live_parlays_v1';
  const PROP_KEYS=['parlay','saved','stake','bookPrice','margin'];
  const POLL_MS=8000, PUSH_MS=400, BOOT_WAIT_MS=6000;
  const st={url:null, conf:null, rev:null, doc:null, applied:false, live:false, ready:false, ok:null, err:null,
    at:null, pending:false, pushing:false, pulls:0, pushes:0};
  const listeners=[];
  ...
  const state=()=>({url:st.url, conf:st.conf, rev:st.rev, applied:st.applied, live:st.live, ready:st.ready, ok:st.ok,
    err:st.err, at:st.at, pending:st.pending, pulls:st.pulls, pushes:st.pushes});
  const ls={
  ...
  async function readConf(){
    try{
      const r=await fetch(CONF+'?t='+Date.now(),{cache:'no-store'});
  ...
  async function getJSON(path){
    const r=await fetch(st.url+path+'?t='+Date.now(),opts());
    if(!r.ok) throw new Error('HTTP '+r.status);
    return r.json();
  }
  ...
  function apply(remote){
    ...
    st.applied=true; st.live=true; st.ok=true; st.err=null;
  ...
  let polling=false;
  async function poll(){
    if(!st.url||polling) return; polling=true;
    try{
      if(!st.applied){ apply(await pull()); st.pulls++; }
        ...
        else { st.ok=true; st.err=null; emit(); }
      }
  ...
  async function push(keepalive){
    ...
    const doc=localDoc();
    if(st.doc&&sig(doc)===sig(st.doc)){ st.pending=false; emit(); return true; }
    const rev=newRev(), at=new Date().toISOString();
    st.pushing=true;
    try{
      const r=await fetch(st.url+'.json?print=silent',opts({method:'PUT',keepalive:!!keepalive,
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({rev,at,doc:{rev,at,json:JSON.stringify(doc)}})}));
      if(!r.ok) throw new Error('HTTP '+r.status);
      st.rev=rev; st.at=at; st.doc=doc; st.pushes++; st.ok=true; st.err=null; st.pending=false;
