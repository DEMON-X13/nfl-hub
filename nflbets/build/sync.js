...
window.NFLSYNC=(function(){
  ...
  const st={url:null, conf:null, rev:null, doc:null, applied:false, live:false, ready:false, ok:null, err:null,
    at:null, checked:null, pending:false, pushing:false, pulls:0, pushes:0};
  const listeners=[];
  ...
  const state=()=>({url:st.url, conf:st.conf, rev:st.rev, applied:st.applied, live:st.live, ready:st.ready, ok:st.ok,
    err:st.err, at:st.at, checked:st.checked, pending:st.pending, pushing:st.pushing, pulls:st.pulls, pushes:st.pushes});
  const ls={
  ...
  async function push(keepalive){
    ...
    try{
      ...
      if(!r.ok) throw new Error('HTTP '+r.status);
      st.rev=rev; st.at=at; st.doc=doc; st.pushes++; st.ok=true; st.err=null; st.pending=false; st.checked=at;
