...
/* Power Ratings: the Impact absences table sits under the ratings, where the ratings it
   ...
   ratings about where the rank tags are measured from goes, and the tier key moves above
   the table. */
function ratingsExtras(){
  ...
  /* the tier key reads before the table it explains, not after thirty-two rows of it */
  const rt=el('ratingsTable'), lg=rt&&rt.querySelector('.tierlegend'), tb=rt&&rt.querySelector('table');
  if(lg&&tb&&lg.compareDocumentPosition(tb)&Node.DOCUMENT_POSITION_PRECEDING){ lg.style.margin='0 0 10px'; rt.insertBefore(lg,tb); }
}
